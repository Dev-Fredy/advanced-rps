...
let playGame = (playerMove) => {
    ...
    document.querySelector('.js_scores').innerHTML = `Wins: ${scores.wins} Ties: ${scores.ties} Loses: ${scores.loses}`
...
let renderHistory = () => {
    ...
    newGameHistory.map((value, index) => {
        ...
        </div> <button class='js_delete_history'>
        Delete</button></div>`
        historyHtml += html
    })
...
let getDayTime = () => {
    ...
    {
        date, thisMonth, actualDay, dayTimeHours, dayTimeMinutes, dayTimeSeconds
    }
...
document.querySelector('.js_footer').innerHTML = `Copyright ${' '} &copy; ${' '}<span class="js_date"></span>${'   '} | All Rights Reserved.`
